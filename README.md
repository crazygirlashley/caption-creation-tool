...
- **Estimated output size** — a live label under the Preview panel shows the estimated final file size as you edit: an exact in-memory encode for static images, and a real GIF/MP4 encode (sampled from a bounded number of frames for large, not-yet-fully-loaded sources) for animations. It turns amber, then red with a warning, once the estimate crosses DeviantArt's 80MB upload cap. Save and Send to DA also check the real output size afterward and warn if it's still too big
- **GIF compression** — a "Compress GIF Output" checkbox, shown only once a GIF is loaded, reveals a color-count slider (256 down to 2) that reduces the palette to shrink the file. Unchecked, or left at 256, it's a no-op; dragging it down updates the live size estimate immediately so you can see the size/quality trade-off before saving, and the same setting is applied to the real Save and Send to DA exports
